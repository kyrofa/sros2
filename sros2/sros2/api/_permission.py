...
def create_permission_file(path, domain_id, policy_element):
    ...
    permissions_xsl = lxml.etree.XSLT(lxml.etree.parse(permissions_xsl_path))
    ...
    if rclpy.get_rmw_implementation_identifier() in _RMW_WITH_ROS_GRAPH_INFO_TOPIC:
        kwargs['allow_ros_discovery_topic'] = lxml.etree.XSLT.strparam('1')
    ...
    with open(path, 'wb') as f:
        f.write(lxml.etree.tostring(permissions_xml, pretty_print=True))
